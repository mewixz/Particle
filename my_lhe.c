...
// Build: g++ my_lhe_reader_non_decayed.c -o my_lhe_reader_non_decayed -I`root-config --incdir` `root-config --libs`
// OR, g++ -std=c++11 my_lhe_reader_non_decayed.c -o my_lhe_reader_non_decayed -I`root-config --incdir` `root-config --libs`
...
#include <TLorentzVector.h>                    // TLorentzVector is a ROOT class for a physical 4-vector | for the description of (r,t), (p,E)
using namespace std;
...
int main(int argc, char **argv)
{
  ...
  {
    ostringstream oss;              //The stringstream class in C++ allows a string object to be treated as a stream. It is used to operate on strings. By treating the strings as streams we can perform extraction and insertion operation from/to string just like cin and cout streams
    oss << "q" << iquark ;
  ...
  //~~~~~~~~~~~~~~~~~~~~~   (opening the .lhe file)    ~~~~~~~~~~~~~~~~~~~~~//

  int nlept=0, nsemi=0, nhadr=0;
  ifstream ff(lhefname.c_str(),ios::in); //ouverture du fichier .lhe  (This ios prefix is used to open a file to read input from the user.)
  ...
  while(!ff.eof())                           // store 8 bytes(64 bits)  |long int stores 4 bytes
  {                                          // while not at eof, continue reading
    std::stringstream buffer;                // buffer is a stringstream object
    ff>>tt;                                  // extraction operator
    buffer << tt << std::endl;               // insert the data from lhe file to stringstream buffer
    line++;
    ...
    if(tt=="<event>")
    {
      ff>>npart>>idprocess>>weight>>scale>>alpha_em>>alpha_s;         // exractng event definition from ff
      ...
      if (weight < 0)
      {
        negativeWeight++;                             // ********  why this command?
        weight = -1;
      }
      ...
      int *Id      = new int[npart+1];      //  new allocates an amount of memory needed to store the object/array(Id) that you request. In this case n numbers of int.
      int *Status  = new int[npart+1];      //  The pointer will then store the address to this block of memory.
      int *Mother1 = new int[npart+1];      //  But be careful, this allocated block of memory will not be freed until you tell it so by writing
      int *Mother2 = new int[npart+1];      //  delete [] array;
      ...
      {                                           //start at one
        ...
        if (Status[i]==-1) continue;          // breaks one itteration | status -1 = initial quark ==> skip
        if (Id[i]==6)  top=i;
        if (Id[i]==-6) topbar=i;
        ...
        //--------met for muon----------------------            if its top 'what about met for muon'
        int id = abs(Id[i]);
        Part_absId = abs(Id[i]);
        ...
        if ( id==14 ) { met=i;}
        //cout << "abs(Id[i]) = " << id << "\t Part_absId = " << Part_absId << "\t Moth_absId = " << Moth_absId <<"\n";
        //cout<<"this is met"<<Id[i]<<endl;
        ////----------------------------------------


        if (abs(Id[Mother1[i]])==24 || abs(Id[Mother1[i]])==6)
        {                                                                 // mother = W
          int id = abs(Id[i]);
        ...
        //  Filling Histograms for extra jets
        ...
        {                                          // bjets
          ...
          bmin=i;
          if(bj[0]==-1) bj[0]=i;                   // *********** what is bj[] defined above, why is it defined
        ...
        //------------------top, top, tbar (ttt~) lep, bJets-----------------------------------
        if (Id[Mother1[i]]==6 || Id[Mother1[i]]==24)
        {                                                                 // mother = W+ coming from top

          if ( Id[i]==-11 || Id[i]==-13 || Id[i]==-15 )
          {                                                                // charged anti leptons
            //if (v[i]->Pt() <= 30 || abs(v[i]->Eta()) > 2.5) continue;
            v_top_lep.SetPxPyPzE(v[i]->Px(),v[i]->Py(),v[i]->Pz(),v[i]->E());  // To set more the one component by one call you can use the SetVect() function for the TVector3 part or SetXYZT(), SetPxPyPzE().
            n_alep++;                                                          // v.SetXYZM(x,y,z,m);   here   ->  v=(x,y,z,e=Sqrt(x*x+y*y+z*z+m*m))
          }

          if ( Id[i]==12 || Id[i]==14 || Id[i]==16 )
          {                                                                 //neutrino
            //if (v[i]->Pt() <= 30 || abs(v[i]->Eta()) > 2.5) continue;
        ...
        {                                                                  // mother = top
          ...
          if ( Id[i]==2 || Id[i]==4 || Id[i]==-1 || Id[i]==-3)
          {                                                                // jets coming from top
            //if (v[i]->Pt() <= 30 || abs(v[i]->Eta()) > 2.5) continue;
      ...
      if ( Id[Mother1[i]]==-6 || Id[Mother1[i]==-24])
        ...
        {                                                                          // mother = tbar
          ...
          {                                                                         // jets coming from tbar
            //if (v[i]->Pt() <= 30 || abs(v[i]->Eta()) > 2.5) continue;
          if (Id[i]==1 || Id[i]==3 || Id[i]==-2 || Id[i]==-4)
      ...
      //~~~~~~~~~~~~~~~~        Jets cuts and filling histograms      ~~~~~~~~~~~~~~~~~~~~~~//
      int n_jets_passing_cuts = 0;
      TLorentzVector v_selected_jets;


      //  Filling the histograms for quarks defined above
